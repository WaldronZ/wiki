...
class WikiWorkflowTest(unittest.TestCase):
    ...
    def make_report_dir(self, tmp: Path) -> Path:
        ...
        (guides_dir / "actions.schema.json").write_text(
        ...
        (guides_dir / "curation.schema.json").write_text(
            (ROOT / "docs" / "guides" / "curation.schema.json").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
        (guides_dir / "queues.schema.json").write_text(
            (ROOT / "docs" / "guides" / "queues.schema.json").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
        (guides_dir / "cohorts.schema.json").write_text(
            (ROOT / "docs" / "guides" / "cohorts.schema.json").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
        for slug, text in {
    ...
    def test_build_validate_and_apply_review_plan(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            self.assertIn('class="home-view-panel"', index_html)
            self.assertIn('class="home-view-menu"', index_html)
            self.assertIn("<summary class=\"button\">视图操作</summary>", index_html)
            ...
            self.assertIn("读论文", index_html)
            self.assertIn("导入论文", index_html)
            self.assertIn("管分类", index_html)
            self.assertIn("更多页面与数据", index_html)
            self.assertIn("命令中心", index_html)
            ...
            self.assertNotIn('<article class="home-lane" data-persona="contributor">', index_html)
            ...
            self.assertIn('"href": "curation.html"', index_html)
            ...
            self.assertIn("Data: queues.json", index_html)
            self.assertIn("Data: cohorts.json", index_html)
            ...
            self.assertIn("Data: curation.json", index_html)
            ...
            self.assertIn("卡片首页", library_html)
            self.assertIn("质量治理", library_html)
            self.assertIn("分类治理", library_html)
            self.assertIn("更多入口与数据", library_html)
            self.assertIn('class="library-secondary-nav"', library_html)
            ...
            self.assertIn('class="library-workspace-panel"', library_html)
            self.assertIn('class="library-workspace-body"', library_html)
            self.assertIn('id="libraryWorkspaceMeta"', library_html)
            self.assertIn("updateWorkspaceMeta", library_html)
            self.assertIn("当前页", library_html)
            self.assertIn("整理工作区", library_html)
            self.assertIn('id="bulkStatus"', library_html)
            self.assertIn('id="bulkPreset"', library_html)
            self.assertIn('id="applyBulkPreset"', library_html)
            self.assertIn('<details class="bulk-panel">', library_html)
            self.assertIn("批量编辑与写回", library_html)
            self.assertIn('class="bulk-panel-body"', library_html)
            ...
            self.assertIn('id="bulkWorkflowMeta"', library_html)
            self.assertIn("updateBulkWorkflowMeta", library_html)
            self.assertIn("presetCompatibility", library_html)
            self.assertIn("workflowFieldValues", library_html)
            self.assertIn("当前状态体系", library_html)
            self.assertIn("preset ${compatiblePresetCount}/${bulkPresets.length} 可用", library_html)
            ...
            self.assertIn("selectedViewName", library_html)
            self.assertIn("当前视图：", library_html)
            self.assertIn("savedView.value = \"\"", library_html)
            self.assertIn("clearActiveFilter", library_html)
            self.assertIn('id="libraryInsights"', library_html)
            self.assertIn('class="library-assist-panel"', library_html)
            self.assertIn("分析与队列建议", library_html)
            self.assertIn('class="library-assist-body"', library_html)
            ...
            self.assertIn('id="queueAdvisor"', library_html)
            self.assertIn('id="advisorCount"', library_html)
            self.assertIn("智能队列建议", library_html)
            self.assertIn("advisorQueueDefinitions", library_html)
            self.assertIn("updateQueueAdvisor", library_html)
            self.assertIn("selectAdvisorQueue", library_html)
            self.assertIn("data-queue-action=\"select\"", library_html)
            self.assertIn("data-queue-action=\"apply\"", library_html)
            self.assertIn("高优先级待读", library_html)
            self.assertIn("缺分类", library_html)
            ...
            self.assertIn('id="libraryMoreActions"', library_html)
            self.assertIn("更多操作", library_html)
            self.assertIn("保存视图、分享筛选和导出当前结果。", library_html)
            self.assertIn('class="action-menu column-menu"', library_html)
            self.assertIn('id="columnMenu"', library_html)
            self.assertIn('id="scanColumns"', library_html)
            self.assertIn('id="fullColumns"', library_html)
            self.assertIn('scan-columns-v1', library_html)
            self.assertIn('data-column-toggle="structure"', library_html)
            self.assertIn('data-column-toggle="structure">结构分类', library_html)
            self.assertIn('data-column-toggle="tags">主题 / 方法', library_html)
            self.assertIn('data-column-toggle="scores">评分', library_html)
            self.assertIn("scanColumnDefaults", library_html)
            self.assertIn("applyColumnPreset(scanColumnDefaults)", library_html)
            self.assertIn("applyColumnPreset(fullColumnDefaults)", library_html)
            self.assertIn('class="row-details"', library_html)
            self.assertIn('class="row-detail-grid"', library_html)
            self.assertIn("<summary>单篇详情</summary>", library_html)
            ...
            self.assertIn("python3 scripts/export_views.py docs --format patch --view <view_id_or_name> --field status --set-value reading --output docs/exports/views-status-patch.csv", views["commands"])
            self.assertIn("python3 scripts/apply_library_metadata.py docs --input docs/exports/views-status-patch.csv --write", views["commands"])
            ...
            self.assertIn('id="viewPatchField"', views_html)
            self.assertIn('id="viewPatchValue"', views_html)
            self.assertIn('id="viewPatchListMode"', views_html)
            self.assertIn('class="button copy-view-json"', views_html)
            self.assertIn('class="button copy-view-patch"', views_html)
            self.assertIn('class="button download-view-patch"', views_html)
            self.assertIn('class="button copy-view-patch-command"', views_html)
            self.assertIn("function patchCsvFor", views_html)
            self.assertIn("function patchCommandFor", views_html)
            ...
            queues = json.loads((report_dir / "queues.json").read_text(encoding="utf-8"))
            self.assertEqual(queues["count"], 2)
            self.assertEqual(queues["queue_count"], 5)
            self.assertIn("queues", queues)
            self.assertIn("top_queues", queues)
            self.assertIn("missing-taxonomy", {item["id"] for item in queues["queues"]})
            self.assertIn("due-review", {item["id"] for item in queues["queues"]})
            self.assertTrue(any(item["preset"] == "schedule_review" for item in queues["queues"]))
            self.assertTrue(any("apply_library_metadata.py" in command for command in queues["commands"]))
            queues_html = (report_dir / "queues.html").read_text(encoding="utf-8")
            self.assertIn("运营队列", queues_html)
            self.assertIn("Queues JSON", queues_html)
            ...
            self.assertIn("split_candidates", cohorts["queues"])
            self.assertIn("singletons", cohorts["queues"])
            self.assertTrue(any(item["pair"] == "研究线 x Method" for item in cohorts["cohorts"]))
            ...
            self.assertIn("Cohorts JSON", cohorts_html)
            self.assertIn('id="cohortSearch"', cohorts_html)
            self.assertIn('id="cohortPair"', cohorts_html)
            self.assertIn('id="cohortAction"', cohorts_html)
            self.assertIn('id="cohortSeverity"', cohorts_html)
            self.assertIn("function renderCohortRows", cohorts_html)
            ...
            self.assertIn("curation.html", {item["href"] for item in catalog["pages"]})
            self.assertIn("queues.html", {item["href"] for item in catalog["pages"]})
            self.assertIn("cohorts.html", {item["href"] for item in catalog["pages"]})
            ...
            self.assertIn("queues.json", catalog["recommended_bootstrap_files"])
            self.assertIn("cohorts.json", catalog["recommended_bootstrap_files"])
            ...
            self.assertIn("facets.json", facets_html)
            curation = json.loads((report_dir / "curation.json").read_text(encoding="utf-8"))
            self.assertEqual(curation["count"], 2)
            self.assertEqual({item["slug"] for item in curation["papers"]}, {"2601.00001-alpha-paper", "2501.00002-beta-paper"})
            self.assertIn("average_score", curation)
            self.assertIn("needs_curation", curation["queues"])
            self.assertIn("missing_review", curation["queues"])
            self.assertIn("research_line", {item["field"] for item in curation["field_contract"]})
            alpha_curation = next(item for item in curation["papers"] if item["slug"] == "2601.00001-alpha-paper")
            self.assertIn(alpha_curation["level"], {"ready", "usable", "needs_curation", "triage"})
            self.assertIn("slug", alpha_curation["patch_columns"])
            self.assertIn("field_status", alpha_curation)
            self.assertTrue(any("apply_library_metadata.py" in command for command in curation["commands"]))
            curation_html = (report_dir / "curation.html").read_text(encoding="utf-8")
            self.assertIn("分类成熟度", curation_html)
            self.assertIn("Curation JSON", curation_html)
            self.assertIn('id="curationSearch"', curation_html)
            self.assertIn('id="curationLevel"', curation_html)
            self.assertIn('id="curationField"', curation_html)
            self.assertIn("function renderCurationRows", curation_html)
            self.assertIn("curation_patch.csv", curation_html)
            ...
            self.assertIn("csv_columns", actions)
            self.assertIn("commands", actions)
            self.assertIn("links", actions)
            self.assertIn("export_actions.py", "\n".join(actions["commands"]))
            self.assertEqual(actions["links"]["html"], "actions.html")
            self.assertIn("command", actions["links"])
            self.assertIn("priority", actions["csv_columns"])
            ...
            self.assertIn("curation.html", {item["href"] for item in manifest["pages"]})
            ...
            self.assertIn("curation.json", {item["href"] for item in manifest["data_files"]})
            self.assertIn("queues.json", {item["href"] for item in manifest["data_files"]})
            self.assertIn("cohorts.json", {item["href"] for item in manifest["data_files"]})
            ...
            self.assertIn("guides/curation.schema.json", {item["href"] for item in manifest["contract_files"]})
            self.assertIn("guides/queues.schema.json", {item["href"] for item in manifest["contract_files"]})
            self.assertIn("guides/cohorts.schema.json", {item["href"] for item in manifest["contract_files"]})
            ...
            self.assertEqual(artifact_by_href["guides/curation.schema.json"]["kind"], "contract")
            self.assertEqual(artifact_by_href["guides/queues.schema.json"]["kind"], "contract")
            self.assertEqual(artifact_by_href["guides/cohorts.schema.json"]["kind"], "contract")
            ...
            self.assertEqual(artifact_by_href["curation.html"]["status"], "ok")
            self.assertRegex(artifact_by_href["curation.html"]["sha256"], r"^[0-9a-f]{64}$")
            self.assertEqual(artifact_by_href["curation.json"]["status"], "ok")
            self.assertRegex(artifact_by_href["curation.json"]["sha256"], r"^[0-9a-f]{64}$")
            self.assertEqual(artifact_by_href["queues.html"]["status"], "ok")
            self.assertRegex(artifact_by_href["queues.html"]["sha256"], r"^[0-9a-f]{64}$")
            self.assertEqual(artifact_by_href["queues.json"]["status"], "ok")
            self.assertRegex(artifact_by_href["queues.json"]["sha256"], r"^[0-9a-f]{64}$")
            self.assertEqual(artifact_by_href["cohorts.html"]["status"], "ok")
            self.assertRegex(artifact_by_href["cohorts.html"]["sha256"], r"^[0-9a-f]{64}$")
            self.assertEqual(artifact_by_href["cohorts.json"]["status"], "ok")
            self.assertRegex(artifact_by_href["cohorts.json"]["sha256"], r"^[0-9a-f]{64}$")
    ...
    def test_invalid_curation_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            report_dir = self.make_report_dir(Path(tmp_name))
            self.run_cmd("scripts/build_wiki.py", str(report_dir))

            (report_dir / "guides" / "curation.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("guides/curation.schema.json: properties.papers is required", result.stderr)
            self.assertIn("guides/curation.schema.json: $defs must be an object", result.stderr)

    def test_invalid_cohorts_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            report_dir = self.make_report_dir(Path(tmp_name))
            self.run_cmd("scripts/build_wiki.py", str(report_dir))

            (report_dir / "guides" / "cohorts.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("guides/cohorts.schema.json: properties.cohorts is required", result.stderr)
            self.assertIn("guides/cohorts.schema.json: $defs must be an object", result.stderr)

    def test_invalid_queues_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            report_dir = self.make_report_dir(Path(tmp_name))
            self.run_cmd("scripts/build_wiki.py", str(report_dir))

            (report_dir / "guides" / "queues.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("guides/queues.schema.json: properties.queues is required", result.stderr)
            self.assertIn("guides/queues.schema.json: $defs must be an object", result.stderr)
    ...
    def test_invalid_actions_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            report_dir = self.make_report_dir(Path(tmp_name))
            self.run_cmd("scripts/build_wiki.py", str(report_dir))

            (report_dir / "guides" / "actions.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
    ...
    def test_invalid_manifest_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            (report_dir / "guides" / "manifest.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
    ...
    def test_invalid_status_schema_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            (report_dir / "guides" / "status.schema.json").write_text(
                json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "properties": {}}),
                encoding="utf-8",
            )
            result = self.run_cmd("scripts/validate_wiki.py", str(report_dir), check=False)
