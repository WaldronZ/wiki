...
class WikiWorkflowTest(unittest.TestCase):
    ...
    def test_build_validate_and_apply_review_plan(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_name:
            ...
            self.assertIn("研究路径", index_html)
            self.assertIn("论文检索", index_html)
            self.assertNotIn("打开命令中心", index_html)
            self.assertNotIn("按场景进入", index_html)
            self.assertNotIn("推荐下一步", index_html)
