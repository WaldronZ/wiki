...
def infer_methods(meta: dict[str, Any]) -> list[str]:
    return sorted(dict.fromkeys(as_list(meta.get("methods"))))
...
def tag_counts(papers: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for paper in papers:
        counter.update(paper["topics"])
        counter.update(paper["methods"])
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .controls {{ display: grid; grid-template-columns: minmax(240px, 1fr) repeat(4, minmax(120px, auto)); gap: 10px; }}
    input, select {{
      width: 100%;
      min-height: 42px;
      border: 1px solid var(--line);
...
<div class="toolbar">
  <div class="shell controls">
    ...
    <select id="topic"><option value="">全部分类</option>{render_topic_options(data["tags"])}</select>
...
function card(p) {{
  ...
  const tags = [...(p.topics || []), ...(p.methods || [])].map(t => `<span class="chip">${{esc(t)}}</span>`).join("");
...
function render() {{
  ...
  const tag = topic.value;
  const codeValue = code.value;
  const importanceValue = Number(importance.value || 0);
  const reviewValue = review.value;
  const today = new Date().toISOString().slice(0, 10);
  const filtered = papers.filter(p => {{
    const text = [p.slug, p.title, p.title_zh, p.title_en, p.arxiv_id, p.excerpt, p.essence, ...(p.authors || []), ...(p.topics || []), ...(p.methods || []), searchTextBySlug.get(p.slug) || ""].join(" ").toLowerCase();
    const tagHit = !tag || [...(p.topics || []), ...(p.methods || [])].includes(tag);
    ...
    return (!q || text.includes(q)) && tagHit && codeHit && importanceHit && reviewHit;
  }});
  cards.innerHTML = filtered.length ? filtered.map(card).join("") : `<div class="empty">没有匹配的论文。</div>`;
}}

[search, topic, code, importance, review].forEach(el => el.addEventListener("input", render));
</script>
"""
...
def render_tags(report_dir: Path, papers: list[dict[str, Any]]) -> None:
    ...
    if grouped:
        ...
        for tag in sorted(grouped, key=lambda name: (-len(grouped[name]), name.lower())):
            ...
            sections.append(f'<section><h2 class="section-title">{html.escape(tag)} <span class="meta">{len(grouped[tag])}</span></h2><ul>{items}</ul></section>')
        content = "\n".join(sections)
    else:
...
<header class="shell">
  <div class="eyebrow">AutoPaperReader Wiki</div>
  <h1>分类总览</h1>
  <p class="lead">按 topic 与 method 汇总论文。分类来自报告 frontmatter，缺失时由构建脚本根据关键词做轻量推断。</p>
  <div class="stats">
