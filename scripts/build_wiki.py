...
function patchCommand(write = false) {{
  return `python3 scripts/apply_library_metadata.py docs --input ~/Downloads/metadata_patch.csv${{write ? " --write" : ""}}`;
}}
...
function buildPatchSpec(showAlerts = false) {{
  ...
  return {{
    ...
    values,
    selected,
    rows: [["slug", ...fields], ...selected.map(row => [row.dataset.slug, ...fields.map(field => values[field])])],
  }};
}}
...
function updateBulkPreview() {{
  ...
  bulkPreview.innerHTML = `
    <div><strong>${{selected.length}}</strong> 篇论文，字段 <strong>${{fields.join(", ")}}</strong></div>
