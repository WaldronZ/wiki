...
`docs/facets.html` 是日常分类工作台：它会按字段展示每个标签命中的论文数、长尾标签、过载标签和动态状态候选值。论文数量变多后，优先从这里判断哪些标签需要合并、哪些大桶需要拆成更细的 track / problem。对应的机器可读任务会写入 `docs/taxonomy_actions.json`，便于接入 issue、看板或桌面软件；`scripts/export_taxonomy_actions.py --format patch` 还能把合并候选导出成可人工确认目标值的写回模板。决定把某个标签或状态重命名之前，可以到 `docs/taxonomy.html` 的「分类变更预览」选择字段、旧值和新值，先查看受影响论文，再下载 `taxonomy_change_patch.csv`，用 `scripts/apply_library_metadata.py` dry-run 或写回。
