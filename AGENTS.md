...
└── docs/                      # 默认的报告目录；用户可在请求中指定其它路径覆盖
    ...
    ├── search_index.json      # 正文全文搜索索引，由脚本自动生成
...
- `<报告目录>/search_index.json`：正文全文搜索索引
- `<报告目录>/index.html`：wiki 首页，支持全文搜索、分类筛选、代码状态筛选、重要性筛选和复习筛选
...
子 agent 的执行规范见 `.Codex/agents/` 下对应文件，你（主线 agent）派发任务前应当默认它们已经被加载，不用把整份说明复制到 prompt 里，但要把 **slug、文件路径、报告结构契约** 这三类关键信息明确传过去。
